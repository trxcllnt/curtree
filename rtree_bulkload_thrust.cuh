...
{
    __host__ __device__
    thrust::tuple<float, float, float, float, int, int> operator()(
            ...
            thrust::tuple<float, float, float, float, int, int>  R)
    {
        ...
        return thrust::tuple<float, float, float, float, int, int>(xmin, ymin, xmax, ymax,
                    pos, count); 
    }
};
...
{
    __host__ __device__
    uint operator()(thrust::tuple<T, T, T, T> t)
    {
...
int build_rtree(BBOX<T>& d_box,RTREE<T>& d_rt) 
{
    ...
    thrust::transform(thrust::make_zip_iterator(thrust::make_tuple(
                      ...
                      d_sort_key.begin(), mbr2key<T>());

    thrust::sort_by_key(
            d_sort_key.begin(), 
            d_sort_key.end(),
            thrust::make_zip_iterator(
                thrust::make_tuple(d_xmin, d_ymin, d_xmax, d_ymax, d_id)));
    ...
    for (int i = 0; i < lev; i++)
    {
        ...
        if (i == 0)
            thrust::reduce_by_key(
                   ...
                   thrust::make_zip_iterator(thrust::make_tuple(d_xmin, d_ymin,
                           d_xmax, d_ymax, thrust::make_counting_iterator<int>(R_sz), ONE)),
                   thrust::make_discard_iterator(), 
                   thrust::make_zip_iterator(thrust::make_tuple(d_R_xmin+start_pos, 
                           d_R_ymin+start_pos, d_R_xmax+start_pos, 
                           d_R_ymax+start_pos, d_R_pos+start_pos, d_R_len+start_pos)),
                   thrust::equal_to<int>(), reduce_op());
        else
            thrust::reduce_by_key(
                   thrust::make_transform_iterator(first, reduce_trans(d)),
                   thrust::make_transform_iterator(last, reduce_trans(d)),
                   thrust::make_zip_iterator(thrust::make_tuple(d_R_xmin+start_pos+levels[i], 
                           d_R_ymin+start_pos+levels[i], d_R_xmax+start_pos+levels[i], 
                           d_R_ymax+start_pos+levels[i], thrust::make_counting_iterator<int>(start_pos+levels[i]), ONE)),
                   thrust::make_discard_iterator(), 
                   thrust::make_zip_iterator(thrust::make_tuple(d_R_xmin+start_pos, 
                           d_R_ymin+start_pos, d_R_xmax+start_pos, 
                           d_R_ymax+start_pos, d_R_pos+start_pos, d_R_len+start_pos)),
...
if(0)
{
    ...
    thrust::copy(levels.begin(),levels.end(),std::ostream_iterator<int>(std::cout, " "));std::cout<<std::endl;    
    
    cout<<"xmin:"<<std::endl;
    thrust::copy(d_R_xmin,d_R_xmin+R_sz,std::ostream_iterator<float>(std::cout, " "));std::cout<<std::endl;
    cout<<"ymin:"<<std::endl;
    thrust::copy(d_R_ymin,d_R_ymin+R_sz,std::ostream_iterator<float>(std::cout, " "));std::cout<<std::endl;
    cout<<"xmax:"<<std::endl;
    thrust::copy(d_R_xmax,d_R_xmax+R_sz,std::ostream_iterator<float>(std::cout, " "));std::cout<<std::endl;
    cout<<"ymax:"<<std::endl;
    thrust::copy(d_R_ymax,d_R_ymax+R_sz,std::ostream_iterator<float>(std::cout, " "));std::cout<<std::endl;
   
    cout<<"pos:"<<std::endl;
    thrust::copy(d_R_pos,d_R_pos+R_sz,std::ostream_iterator<int>(std::cout, " "));std::cout<<std::endl;
    cout<<"len:"<<std::endl;
