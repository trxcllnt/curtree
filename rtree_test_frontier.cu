...
int main(int argc, char *argv[]) {
  ...
  if (0) {
    ...
    thrust::copy(h_d_box.id, h_d_box.id + h_d_box.sz, std::ostream_iterator<int>(std::cout, " "));
    std::cout << std::endl;

    std::cout << "main: d_r_box.id" << std::endl;
    thrust::device_ptr<int> rid_ptr = thrust::device_pointer_cast(d_r_box.id);
    thrust::copy(rid_ptr, rid_ptr + d_r_box.sz, std::ostream_iterator<int>(std::cout, " "));
    std::cout << std::endl;
  }
