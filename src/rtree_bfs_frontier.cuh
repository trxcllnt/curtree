...
struct invalid_result {
  __host__ __device__ bool operator()(thrust::tuple<int, int> t) { return thrust::get<0>(t) == -1; }
};

struct is_invalid {
  __device__ __host__ bool operator()(char flag) { return flag > 0; }
};
...
#define BATCH \
  32  // number of query windows, i.e., 1*q_size; the larger, the higher parallelism and faster
...
__global__ void query_tree_bfs_frontier(int size,
                                        int num_blocks,
                                        float *q_xmin,
                                        float *q_ymin,
                                        float *q_xmax,
  ...
  if (idx_block < num_blocks) {
    ...
    while (!done && !overflow) {
      ...
      if (r_node >= 0 && id >= 0 && R_pos[r_node] < R_size) {
        ...
        for (int i = R_pos[r_node]; i < R_pos[r_node] + R_len[r_node]; i++) {
          if (intersect(xmin, ymin, xmax, ymax, R_xmin[i], R_ymin[i], R_xmax[i], R_ymax[i])) {
            children++;
          }
        }
      }
      ...
      for (int offset = 1; offset < blockDim.x; offset <<= 1) {
        ...
        if (threadIdx.x >= offset) val += scan[threadIdx.x - offset];
        __syncthreads();
        // store
      ...
      if (threadIdx.x == blockDim.x - 1) {
        if (scan[QUEUE - 1] >= QUEUE) overflow = true;
        if (scan[QUEUE - 1] == 0) done = true;
      }
      __syncthreads();
      if (!done && !overflow) {
    ...
    for (int i = 0; i < QUEUE / blockDim.x; i++)
      if (threadIdx.x + i * blockDim.x < QUEUE) {
        q_out_nodes[idx_block * QUEUE + threadIdx.x + i * blockDim.x] =
          nodes[threadIdx.x + i * blockDim.x];
        q_out_ids[idx_block * QUEUE + threadIdx.x + i * blockDim.x] =
          q_ids[threadIdx.x + i * blockDim.x];
      }
...
struct offset_functor {
  ...
  __host__ __device__ int operator()(int val) { return pos[val] - offset; }
};
...
struct count_hit {
  ...
  __host__ __device__ int operator()(thrust::tuple<int, int> t) {
    ...
    for (int i = rid; i < fanout + rid; i++) {
      if (i >= size) break;
      if (intersect(
            qxmin[qid], qymin[qid], qxmax[qid], qymax[qid], rxmin[i], rymin[i], rxmax[i], rymax[i]))
        hit++;
    }
    return hit;
  }
...
struct write_hit {
  ...
  __host__ __device__ int operator()(thrust::tuple<int, int, int> t) {
    int hit = 0;
    int qid = thrust::get<0>(t);
    int rid = thrust::get<1>(t);
...
// d_dq: (d,q)<==>(f,t)
template <class T>
void bfs_frontier(const RTREE<T> &d_rt,
                  const BBOX<T> &d_rbox,
                  const BBOX<T> &d_qbox,
                  IDPAIR &d_dq) {
  ...
  if (0) {
    ...
    thrust::copy(d_R_Pos, d_R_Pos + r_size, std::ostream_iterator<int>(std::cout, " "));
    std::cout << std::endl;
    cout << "bfs_frontier: len:" << std::endl;
    thrust::copy(d_R_Len, d_R_Len + r_size, std::ostream_iterator<int>(std::cout, " "));
    std::cout << std::endl;
    ...
    cout << "xmin:" << std::endl;
    thrust::copy(d_R_xmin, d_R_xmin + r_size, std::ostream_iterator<float>(std::cout, " "));
    std::cout << std::endl;
    cout << "ymin:" << std::endl;
    thrust::copy(d_R_ymin, d_R_ymin + r_size, std::ostream_iterator<float>(std::cout, " "));
    std::cout << std::endl;
    cout << "xmax:" << std::endl;
    thrust::copy(d_R_xmax, d_R_xmax + r_size, std::ostream_iterator<float>(std::cout, " "));
    std::cout << std::endl;
    cout << "ymax:" << std::endl;
    thrust::copy(d_R_ymax, d_R_ymax + r_size, std::ostream_iterator<float>(std::cout, " "));
    std::cout << std::endl;
  }
  ...
  query_tree_bfs_frontier<<<grid, block>>>(q_size,
                                           num_blocks,
                                           d_qbox.xmin,
                                           d_qbox.ymin,
                                           d_qbox.xmax,
  ...
  int invalid_blocks_sz = thrust::copy_if(thrust::make_counting_iterator(0),
                                          thrust::make_counting_iterator(0) + num_blocks,
                                          d_q_out_flag_ptr,
                                          invalid_blocks.begin(),
                                          is_invalid()) -
                          invalid_blocks.begin();
  ...
  if (invalid_blocks_sz > 0) {
    std::cout << "shared memory overflew; a DFS module is needed for overflown blocks......."
              << std::endl;
  ...
  result_sz =
    thrust::remove_if(thrust::make_zip_iterator(thrust::make_tuple(d_q_nodes_ptr, d_q_ids_ptr)),
                      thrust::make_zip_iterator(
                        thrust::make_tuple(d_q_nodes_ptr + result_sz, d_q_ids_ptr + result_sz)),
  ...
  // thrust::copy(d_q_nodes_ptr,d_q_nodes_ptr+result_sz,std::ostream_iterator<int>(std::cout, "
  // "));std::cout<<std::endl;

  thrust::transform(
    d_q_nodes_ptr, d_q_nodes_ptr + result_sz, d_q_nodes_ptr, offset_functor(d_rt.pos, d_rt.sz));
  ...
  if (0) {
    ...
    thrust::copy(rid_ptr, rid_ptr + result_sz, std::ostream_iterator<int>(std::cout, " "));
    std::cout << std::endl;
    std::cout << "qid_ptr:" << std::endl;
    thrust::copy(qid_ptr, qid_ptr + result_sz, std::ostream_iterator<int>(std::cout, " "));
    std::cout << std::endl;
  }
  gettimeofday(&t0, NULL);
  // std::cout<<"d_rt.sz="<<d_rt.sz<<"d_rbox.sz="<<d_rbox.sz<<"  result_sz="<<result_sz<<std::endl;
  thrust::transform(
    thrust::make_zip_iterator(thrust::make_tuple(qid_ptr, rid_ptr)),
    thrust::make_zip_iterator(thrust::make_tuple(qid_ptr + result_sz, rid_ptr + result_sz)),
    d_count.begin(),
    count_hit(d_qbox.xmin,
              d_qbox.ymin,
              d_qbox.xmax,
              d_qbox.ymax,
              d_rbox.xmin,
  ...
  if (0) {
    ...
    thrust::copy(d_count.begin(), d_count.end(), std::ostream_iterator<int>(std::cout, " "));
    std::cout << std::endl;
  }
  ...
  if (0) {
    ...
    thrust::copy(rid_ptr, rid_ptr + d_rbox.sz, std::ostream_iterator<int>(std::cout, " "));
    std::cout << std::endl;
  }
  ...
  thrust::transform(
    thrust::make_zip_iterator(thrust::make_tuple(qid_ptr, rid_ptr, d_count.begin())),
    thrust::make_zip_iterator(
      thrust::make_tuple(qid_ptr + result_sz, rid_ptr + result_sz, d_count.end())),
    thrust::make_discard_iterator(),
    write_hit(d_qbox.xmin,
              d_qbox.ymin,
              d_qbox.xmax,
              d_qbox.ymax,
              d_rbox.xmin,
  ...
  if (0) {
    ...
    thrust::copy(fid_ptr, fid_ptr + total_hit, std::ostream_iterator<int>(std::cout, " "));
    std::cout << std::endl;
    std::cout << "tid_ptr:" << std::endl;
    thrust::copy(tid_ptr, tid_ptr + total_hit, std::ostream_iterator<int>(std::cout, " "));
    std::cout << std::endl;
  }
}
